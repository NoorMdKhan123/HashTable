using System;

namespace HashTableExample
{
     class Program
    {
        static void Main(string[] args)
        {
            MyMapNode<string, string> hash = new MyMapNode<string, string>(14);
            //hash.Add("0", "to");
            //hash.Add("1", "be");
            //hash.Add("2", "or");
            //hash.Add("3", "not");
            //hash.Add("4", "to");
            //hash.Add("5", "be");

            ////foreach (string key in hash)
            ////{
            ////    Console.WriteLine(String.Format("{0}: {1}", key, hashTabl[key]));
            ////}

            //Console.WriteLine("5th index value : " + hash5);
            
            hash.Add("0", "Paranoids");
            hash.Add("1", "are");
            hash.Add("2", "not");
            hash.Add("3", "Paranoids");
            hash.Add("4", "becuase");
            hash.Add("5", "they");
            hash.Add("6", "keep");
            hash.Add("7", "putting");
            hash.Add("8", "themselves");
            hash.Add("9", "deliberately");
            hash.Add("10", "into");
            hash.Add("11", "paranoid");
            hash.Add("12", "avoidable");
            hash.Add("13", "situations");

            string hash0 = hash.Get("0");
            Console.WriteLine("0th index value : " + hash0);
            string hash1 = hash.Get("1");
            Console.WriteLine("1st index value : " + hash1);
            string hash2 = hash.Get("2");
            Console.WriteLine("2nd index value : " + hash2);
            string hash3 = hash.Get("3");
            Console.WriteLine("3rd index value : " + hash3);
            string hash4 = hash.Get("4");
            Console.WriteLine("2nd index value : " + hash4);
            string hash5 = hash.Get("5");
            Console.WriteLine("5th index value : " + hash5);
            string hash6 = hash.Get("6");
            Console.WriteLine("0th index value : " + hash6);
            string hash7 = hash.Get("7");
            Console.WriteLine("1st index value : " + hash7);
            string hash8 = hash.Get("8");
            Console.WriteLine("2nd index value : " + hash8);
            string hash9 = hash.Get("9");
            Console.WriteLine("3rd index value : " + hash9);
            string hash10 = hash.Get("10");
            Console.WriteLine("2nd index value : " + hash10);
            string hash11 = hash.Get("11");
            Console.WriteLine("11th index value : " + hash11);
            string hash12 = hash.Get("12");
            Console.WriteLine("12th index value : " + hash12);
            string hash13 = hash.Get("13");
            Console.WriteLine("13th index value : " + hash13);


        }
    }
}
